...
public class SessionManager implements Player.Callback {
    ...
    public PlaylistItem add(Uri uri, String mime, PendingIntent receiver) {
        return add(uri, null, mime, receiver, null, null);
    }

    public PlaylistItem add(Uri uri, Bitmap pic, String mime, PendingIntent receiver, String title, String subtitle) {
        if (DEBUG) {
        ...
        PlaylistItem item = new PlaylistItem(
                Integer.toString(mSessionId), Integer.toString(mItemId), uri, pic, mime, receiver, title, subtitle);
        mPlaylist.add(item);
        mItemId++;
    ...
    public PlaylistItem seek(String iid, int pos) {
        if (DEBUG) {
    ...
    public void onItemReady(boolean hasVideo){
        // Notify activity to update video layout
        if (mCallback != null) {
            mCallback.onItemReady(hasVideo);
        }
    }
