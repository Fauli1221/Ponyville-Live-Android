...
public class RemotePlayer extends Player {
        ...
        mClient.getStatus(item.getRemoteItemId(), null, new ItemActionCallback() {
            ...
            public void onResult(Bundle data, String sessionId, MediaSessionStatus sessionStatus,
                ...
                if (state == MediaItemStatus.PLAYBACK_STATE_PLAYING
                    ...
                    item.setState(state);
                    item.setPosition((int)itemStatus.getContentPosition());
                    item.setDuration((int)itemStatus.getContentDuration());
    ...
    public void updateTrackInfo() {
        ...
        mTrackInfo = "";
        mSnapshot = null;
        Intent intent = new Intent(OverlayRouteProvider.ACTION_GET_TRACK_INFO);
        intent.addCategory(OverlayRouteProvider.CATEGORY_OVERLAY_ROUTE);
        if (mRoute != null && mRoute.supportsControlRequest(intent)) {
            ControlRequestCallback callback = new ControlRequestCallback() {
                ...
                public void onResult(Bundle data) {
                    ...
                    if (data != null) {
                        mTrackInfo = data.getString(OverlayRouteProvider.TRACK_INFO_DESC);
                        mSnapshot = data.getParcelable(
                                OverlayRouteProvider.TRACK_INFO_SNAPSHOT);
                    }
                }
                @Override
